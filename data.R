...
for (yardline_range in names(yardline_bins)) {
  ...
  for (input_down in 1:4) {
    for (yards in 1:20) {
      ...
      # Extract real rush data with time filter (5+ minutes left in half)
      rush_yards <- pbp_data %>%
        filter(
          play_type == "run",
          qb_scramble == 0,
          ...
          game_seconds_remaining >= 300  # Filter plays with at least 5 min left in half
        ) %>%
      ...
      # Extract real pass data with time filter
      pass_yards <- pbp_data %>%
        filter(
