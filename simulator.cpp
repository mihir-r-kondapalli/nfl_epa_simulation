        ...
        for (down = 4; down > 0; down--) {
            for (yards_to_go = 1; yards_to_go <= 20; yards_to_go++) {
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    epa_punt_val += (down == 4) ? get_epa_punt_val(yardline, mt1) : -1000;   // only viable if it is 4th down
                }
                ...
                if (epa_rush_val > 1e10 || epa_pass_val > 1e10) {
                    cerr << "Large EPAs encounted (Re-run program): " << epa_rush_val << ", " << epa_pass_val << endl;
                    exit(1);
                }
...
int main(int argc, char* argv[]) {
    ...
    saveDataToCSV(target_data+"/run_eps.csv", run_epas);
    saveDataToCSV(target_data+"/pass_eps.csv", pass_epas);
    saveDataToCSV(target_data+"/kick_eps.csv", kick_epas);
    saveDataToCSV(target_data+"/punt_eps.csv", punt_epas);
    saveDataToCSV(target_data+"/max_eps.csv", max_epas);
